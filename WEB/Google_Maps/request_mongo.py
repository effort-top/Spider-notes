...
import asyncio
import traceback
import motor.motor_asyncio  # 引入 motor 异步驱动
import aiohttp
import pandas as pd
...
cookies = {
    ...
    "AEC": "AZ6Zc-X4sz-4bRAD5xSHAoCHE5rmnnThf8Nit5Mu7N8gnPbwPZj3uz4THlY",
    "NID": "519=VTB_Hh6nJ3xE_Psk0twKgR9rt-vbYJ1etlOcThzqIR4bfrMJ7iqmfef14fsCey_ntIh1VBD_Pc5V57y-DGctX-hwoisstcyz0sFhWPHaQvxCxAH6YUP0O-e9dQq4xMA55FluMJrb9ZBpgaGwaq7H0nChtbz9pu_e6m0UEzdAiy7zOd9Hs8xj-I7wE0cxwJBYDiPqs-ARPiqrxcOyxJzaAju1crYUaqxtk1acrxNKnSG7k5Hehv2mALAuv6oP6lT0egpFIHxvxYEb5Sx21G6sisi8mISsMHq-w0sCr3eDN5P6jOAhc3tZI340hX5f1mjh9E3dDS3ojR3ejUtpCGBs_V1-dlr946y6OF2JNPaGU7KZe4m9Kw-ZWY22VlhdeWvUqDzFwdnm4h-iNas1yB2aFyyPgQQQMtyBKRHrCmCdYl7Q7ioatH9aYfhO5p94xwnlVyhAR3dZ9iyYXry0O-0oSgsIpFQg18BidIi0ZdDijlrEyGqNeli4kNDNqxz6EqJnTRC1BElLOq0YwJ4mF8eYxXLUa7kqVzVr65EYDb7LOQJG_oPq3ZifOHOrCpjwXSw-Ohu7-tmPXnCxXtrQnVIjpxNlu6bhG_mKJHz4N6PWKa-T51yEk0BzpJWuTDAvHshul7GV9bo0H6F35PYBhSrGN8C7wkuNv2QDDlYQOTbGHAQpW4Hp4KIdnYZw6WIyO2iQ4Q1nT4IaB445fC17cd5kEsDRlHQ3HyWXE4I_Mc_S_E99sYxKXwdoftcU3CuSQWVQ1IpB9E_0VUp5swbbIrmo",
    "UULE": "a+cm9sZTogMQpwcm9kdWNlcjogMTIKdGltZXN0YW1wOiAxNzMyNzgwNTk3NTc5MDAwCmxhdGxuZyB7CiAgbGF0aXR1ZGVfZTc6IDM1NjA5MTk3MwogIGxvbmdpdHVkZV9lNzogMTM5NzMwMzM2NAp9CnJhZGl1czogMjY3ODQxMS4zMzQzODc5Njk3CnByb3ZlbmFuY2U6IDYK",
    "__Secure-1PSIDTS": "sidts-CjIBQT4rXzBzwZ-ICyIKTDY07J2AO8xVY_t6QBv_QUrhS6xicBsYYqMOkoc-GbxnuQpEYhAA",
    "__Secure-3PSIDTS": "sidts-CjIBQT4rXzBzwZ-ICyIKTDY07J2AO8xVY_t6QBv_QUrhS6xicBsYYqMOkoc-GbxnuQpEYhAA",
    "SIDCC": "AKEyXzUyBgGxoss3YDMMYdTgeRavVyVqqarATMOPM5mzKnDecApHDvd9r1ZeqVSmH5XN9S0MGx0",
    "__Secure-1PSIDCC": "AKEyXzXsJqgBBVtqqx0GMV1Ku4-_ysPY3jydV7ldnyUx40PR58NEDUk9JKC90mGEK3XBRjF4NQ",
    "__Secure-3PSIDCC": "AKEyXzVdeFU8Q5CmeXNUUMIScBnt9Ct1p2SCsDZuWfy1rq0e3AWo0n7P19yJFM3WeQjs-rJhoIE"
}
...
def worker(task_queue,finished_count):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if extracted_data is None:
                    ...
                    task['try_num'] += 1  # 增加重试次数
                    task_queue.put(task)  # 将任务重新放回队列
                    continue

                # 将解析结果保存到 MongoDB
                try:
                    # 更新任务状态为 completed 并保存解析结果
                    collection.update_one(
                        {"fid": task_id},
                        {"$set": {
                            "status": "finished",
                    ...
                    logger.error(f"FID: {task['fid']} | 保存结果到 MongoDB 时出错: {e}")
...
async def worker1(task_queue, finished_count):
    """
    异步 worker，处理任务队列中的任务
    """
    client = motor.motor_asyncio.AsyncIOMotorClient(MONGO_URI)  # 使用异步 MongoDB 客户端
    db = client[DB_NAME]
    collection = db[COLLECTION_NAME]
...
def success_count_monitor(completed_count):
    """
    ...
    while True:
        ...
        if completed_count.value == last_completed_count:
            logger.info(f"-------------已运行 {elapsed_time} 分钟 | 完成的任务数量未变化: {completed_count.value} | 最后一次变化的执行时间 {last_change_time} ---------------")
        else:
            # 如果任务数量发生变化，记录新的任务数量和时间
            last_completed_count = completed_count.value
            last_change_time = time.time()  # 更新最后一次变化的时间
            logger.success(f"-------------已运行 {elapsed_time} 分钟 | 完成的任务数量: {completed_count.value}---------------")

def task_maker(task_queue, collection):
    """
    ...
    while True:
        ...
        try:
            ...
            # 检查任务是否足够
            if len(tasks) < 30:
                logger.warning(f"从 MongoDB 中获取的 'unfinished' 任务数量不足 30 个，仅选择 {len(tasks)} 个任务。")
...
def main():
    ...
    completed_count = multiprocessing.Value('i', 0)  # 存储已完成的任务数量
    ...
    # 启动任务生成器线程
    task_maker_thread = threading.Thread(target=task_maker, args=(task_queue, collection))
    task_maker_thread.start()

    processes = []
    num_workers = 2
    for _ in range(num_workers):
        p = multiprocessing.Process(target=worker, args=(task_queue, completed_count))
        processes.append(p)
        p.start()
        # p = multiprocessing.Process(target=run_worker_in_process, args=(task_queue, completed_count))
        # processes.append(p)
        # p.start()

    # 启动成功任务计数监控进程
    monitor_process = multiprocessing.Process(target=success_count_monitor, args=(completed_count,))
    ...
    # 向队列中放入停止信号，告诉工作进程停止
    for _ in range(num_workers):
        task_queue.put(None)  # 向队列中放入 None，作为停止信号
