function replace_thread() {
    ...
    Interceptor.replace(pthread_create_addr, new NativeCallback((parg0, parg1, parg2, parg3) => {
        ...
        if ((so_name.indexOf("libexec.so") > -1) || (so_name.indexOf("xxxx.so") > -1)) {
            var offsetArr = [274972, 275056] // add thread offset
            if (offsetArr.includes(offset)) {
                console.log("hook at ", offset)
            } else {
...
setTimeout(function () {
    Java.perform(function () {
        ...
        nc.hasTransport.implementation = function (param) {
            console.log('hook hasTransport');
            return false;
        }
        nc.hasCapability.implementation = function (param) {
            console.log('hook hasCapability');
            return true;
        }

        var ni = Java.use("android.net.NetworkInfo");
        ni.isConnected.implementation = function (param) {
            console.log('hook isConnected');
            return false;
        }

        var nic = Java.use('java.net.NetworkInterface');
        nic.getName.implementation = function (param) {
...
// frida -U -f com.lovexyx2020.test  -l bypass_pthread_create.js -o test.log
// frida -U -f com.hupu.shihuo  -l bypass_pthread_create.js -o shihuo.log
